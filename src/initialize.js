...
instance.data.loadDAS = (das) => {
    das.forEach((das, index) => {
        ...
        var createCoord = instance.data.getStartCoordinates(
...
instance.data.createExistingRect = (createCoord, color, name) => {
    // Create graphics
    ...
    instance.data.currentRectangle.position.copyFrom(new PIXI.Point(createCoord.startRectX,
    ...
    instance.data.addLabel(instance.data.currentRectangle);
    console.log("currentRect", instance.data.currentRectangle);
    instance.data.currentRectangle = null;
...
instance.data.controlOut = () => {
    e.target.isOver = false;
    instance.data.removeIfUnused(e.target)
}
...
instance.data.onRectangleOver = () => {
    ...
    buttonMove.x = e.target.x + e.target.width - 18;
    buttonMove.y = e.target.y + 23;
    ...
    buttonScale.x = e.target.x + e.target.width - 20;
    buttonScale.y = e.target.y + e.target.height - 20;
...
instance.data.scaleRect = (resizeRectange, instance.data.dragController) => {
    //currentPosition)
    let {
        start,
        size
    } = instance.data.getStartAndSize(instance.data.startPosition, instance.data.dragController.position, "scaleRect")
    ...
    let startPositionController = new PIXI.Point(instance.data.dragController.position.x - 20, instance.data.dragController.position.y - 20);
    ...
    instance.data.dragController.position.copyFrom(startPositionController)
    console.log("scaleRect, startPos, startPosController,instance.data.dragController,mainContainer",instance.data.startPosition,startPositionController,instance.data.dragController.position,mainContainer.position);
}
...
instance.data.moveRect = (resizeRectange, instance.data.dragController) => {
    // Move control is on right side
    // and our rect is anchored on the left we subtract width of rect
    instance.data.startPosition = new PIXI.Point(instance.data.dragController.position.x - resizeRectange.width, instance.data.dragController.position.y);
    let startPositionController = new PIXI.Point(instance.data.dragController.position.x - 18, instance.data.dragController.position.y + 23);
...
instance.data.onRectangleOver = () => {
    ...
    instance.data.inputMode = instance.data.inputModeEnum.select
    // set current hovered rect to be on the top
    instance.data.bringToFront(e.target)
    // Create button move
    const buttonMove = new PIXI.Sprite(instance.data.textureMove);
    // set anchor middle
    buttonMove.anchor.set(0.5);
