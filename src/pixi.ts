...
function loadDAS(das) {
    das.forEach((das, index) => {
        ...
        createExistingRect(createCoord, "DE3249", "temp");
...
.drawRect(createCoord.startRectX,
  ...
  createCoord.height)
.endFill()
currentRectangle.labelColor = color;
currentRectangle.oldColor = color;
currentRectangle.name = name;
...
function onRectangleOver() {
    ...
    // so we know which control is pressed
...
function scaleRectB(resizeRectange, currentPosition) {
    let {
        start,
        size
    } = getStartAndSize(startPosition, currentPosition)
